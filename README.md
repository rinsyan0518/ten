...
`.Ten.Home`/`.Ten.DotfilesRoot` embed a local absolute path, which may contain your OS username — avoid rendering them into files you publish.
...
templates = { "home:.gitconfig.local" = "git/gitconfig.local.tmpl" } # rendered via text/template ({{ .Vars.key }} / {{ .Ten.key }})
